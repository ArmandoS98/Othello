...
public class Board {
    ...
    // Coloca el ajedrez en el tablero [fila] [col]
    ...
    // Compruebe la ficha si es un posible movimiento.
    ...
    public boolean flip(int currentRow, int currentCol) {
        ...
        for (int dirRow = -1; dirRow < 2; dirRow++) {
            for (int dirCol = -1; dirCol < 2; dirCol++) {
                ...
                // Ignorar la dirección cero (posición actual)
                if (dirRow == 0 && dirCol == 0) {
                    continue;
                }
                ...
                // Compruebe el tablero [newRow] [newCol] en el tablero
                if (newRow >= 0 && newRow <= 7 && newCol >= 0 && newCol <= 7) {

                    // devuelve true si board [newRow] [newCol] es de color opuesto a jugadorActual
                    COLOR oppoColor = this.currentPlayer.getColor() == COLOR.WHITE ? COLOR.BLACK : COLOR.WHITE;
                    ...
                    if (board[newRow][newCol] == oppoColor) {
                        for (int range = 0; range < 8; range++) {
                            ...
                            // Saltar el caso fuera del tablero.
                            if (nRow < 0 || nRow > 7 || nCol < 0 || nCol > 7) {
                                continue;
                            }

                            // Compruebe si podemos voltear en esta dirección
                            if (board[nRow][nCol] == this.currentPlayer.getColor()) {
    ...
    // Devuelve el número de espacios vacíos en el tablero
    public int checkEnd() {
        int slotsLeft = 0;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
